...
@section('content')
    @isset($set->image_url)
        <a class="h-96 w-full rounded-t overflow-hidden block" href="{{ $set->image_url }}" target="_blank">
            <img alt="{{ "Afbeelding van {$set->name}" }}" src="{{ $set->image_url }}" class="object-contain h-full w-full">
        </a>
    @endisset
    ...
    <div class="p-4">
        ...
        <form method="post" action="{{ route('sets.destroy', [\Illuminate\Support\Facades\Request::segment(2), $set->id]) }}">
            @method('DELETE')
            @csrf
